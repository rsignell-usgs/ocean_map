...
var Albers = function() {
  ...
  var phi1 = radians(35.0);
  var phi2 = radians(46.0);
  var n = .5 * (phi1 + phi2);
	var C = Math.cos(phi1) * Math.cos(phi1) + 2 * n * Math.sin(phi1);
	var phi0 = radians(40.0);
	var lambda0 = radians(-65.0);
...
var MotionDisplay = function(canvas, imageCanvas, field, numParticles, opt_projection) {
  ...
  this.field = field;
	...
	this.speedScale = 1.5;   // controls the speed of the streaks
...
function init() {
	...
	day = day.replace('September', 'Sep'); // No room for full "September".
	document.getElementById('update-time').innerHTML =
	   '<span id="day">' + day +'</span><br>' + time + ' EST' +
		 '<br><span id="time-explanation">(mid-time for 24 hour average)</span>';
...
3400., 0., canvas.height, -75.9, 35.1);
